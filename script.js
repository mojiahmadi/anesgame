...
function closeAchievementsModal() {
    ...
    let tapSelectedElement = null; // آیتم انتخاب‌شده در حالت لمس-برای-جایگذاری (موبایل)
...
function goToLogin() {
    ...
    function actuallyStartGame(levelIndex) {
        ...
        const hintTextEl = document.getElementById('panel-hint-text');
        if (hintTextEl) {
            hintTextEl.innerText = isMobileInput
                ? 'روی یک گزینه بزن، بعد روی جای درستش بزن.'
                : 'گزینه‌ها را بکشید و در جای درست رها کنید.';
        }
        ...
        if (isMobileInput) {
            if (!dc.dataset.tapBound) {
                ...
                dc.dataset.tapBound = '1';
            }
            if (!dtc.dataset.tapBound) {
                dtc.addEventListener('click', handleZoneTapClick);
                dtc.dataset.tapBound = '1';
            }
        }
    ...
    function handleLayerTapClick(e) {
        ...
        if (tapSelectedElement === item) {
            ...
            tapSelectedElement = null;
            setTapZonesArmed(false);
            return;
        }

        if (tapSelectedElement) {
            tapSelectedElement.classList.remove('tap-selected');
        }

        tapSelectedElement = item;
        item.classList.add('tap-selected');
        setTapZonesArmed(true);
    }

    function handleZoneTapClick(e) {
        const zone = e.target.closest('.drop-zone');
        if (!zone || zone.classList.contains('correct') || !tapSelectedElement) return;

        const expectedId = zone.getAttribute('data-expected-id');
        draggedElement = tapSelectedElement; // استفاده از همون منطق مشترک processDropResult

        tapSelectedElement.classList.remove('tap-selected');
        const placedItem = tapSelectedElement;
        tapSelectedElement = null;
        setTapZonesArmed(false);

        processDropResult(zone, placedItem.id, expectedId);
    }

    function setTapZonesArmed(armed) {
        document.querySelectorAll('.drop-zone:not(.correct)').forEach(z => z.classList.toggle('tap-armed', armed));
    }
    ...
    function handleTouchMove(e) {
        ...
        const dropZone = findIntersectingDropZone(floatingTouchEl);
    ...
    function findIntersectingDropZone(floatingEl) {
        ...
        const dropZones = document.querySelectorAll('.drop-zone:not(.correct)');
        for (let zone of dropZones) {
            const zoneRect = zone.getBoundingClientRect();
           
            if (
                floatCenterX >= zoneRect.left &&
                floatCenterX <= zoneRect.right &&
                floatCenterY >= zoneRect.top &&
                floatCenterY <= zoneRect.bottom
            ) {
                return zone;
            }
        }
    ...
    if (droppedId === expectedId) {
        ...
        draggedElement.classList.remove('wrong-shake'); 
        ...
        const statBox = timerDisplayEl.closest('.stat-box');
        if (statBox) statBox.classList.add('timer-alert');

        const currentWrongPiece = draggedElement; 
        
        if (currentWrongPiece) {
            currentWrongPiece.classList.add('wrong-shake');
        }
        
        setTimeout(() => {
            dropZoneElement.classList.remove('error');
            if (statBox) statBox.classList.remove('timer-alert');
            if (currentWrongPiece) currentWrongPiece.classList.remove('wrong-shake'); 
        }, 500);
    }
}
    ...
    function handleTimeUp() {
        ...
        document.querySelectorAll('.layer-item:not([draggable="false"])').forEach(el => {
            el.setAttribute('draggable', 'false');
            el.style.opacity = '.4';
            el.style.cursor = 'not-allowed';
            el.classList.remove('tap-selected');
        });
        document.querySelectorAll('.drop-zone').forEach(dz => dz.classList.remove('drag-over', 'tap-armed'));
        tapSelectedElement = null;
